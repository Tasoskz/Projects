...
fprintf('Proportional Gain:  %.2f 10^-3\n', alpha*1e3);
fprintf('Integral Gain:  %.2f 10^-3\n', rho*1e3);

% MAGNITUDE RESPONSE
figure('Name', 'Closed-Loop Magnitude Response', 'Color', 'w');
semilogx(f, 20*log10(abs(H_TDC_REF)), 'b', 'LineWidth', 1.5); 
hold on;
semilogx(f, 20*log10(abs(H_DCO)), 'r', 'LineWidth', 1.5);
...
xlabel('Offset Frequency (Hz)');
ylabel('Magnitude (dB)');
title('Closed-Loop Magnitude', 'FontSize', 13, 'Color', 'k');
lgd = legend('H_{TDC/REF} (Low-Pass)', 'H_{DCO} (High-Pass)', ...
    'Location', 'southwest');
ylim([-40 40]); 
set(lgd, 'TextColor', 'k', 'Color', 'w', 'EdgeColor', 'k');

% FIGURE 2: PHASE RESPONSE
figure('Name', 'Closed-Loop Phase Response', 'Color', 'w');
semilogx(f, angle(H_TDC_REF)*180/pi, 'b', 'LineWidth', 1.5); 
hold on;
semilogx(f, angle(H_DCO)*180/pi, 'r', 'LineWidth', 1.5);
...
title('Closed-Loop Phase Response', 'FontSize', 13, 'Color', 'k');
lgd = legend('\angle H_{TDC/REF}', '\angle H_{DCO}', 'Location', ...
    'southwest');
yticks(-180:45:180);
set(lgd, 'TextColor', 'k', 'Color', 'w', 'EdgeColor', 'k');
